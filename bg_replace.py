...
from export_model import download_export_model
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='Video folder path for inference',
        type=str,
        default=None)
    parser.add_argument(
        '--bg_img_path',
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(args.cfg):
        download_export_model.download(args.use_cpu)

    if args.video_folder_path:
        from glob import glob
        import os

        for path in glob(os.path.join(args.video_folder_path, '*')):
